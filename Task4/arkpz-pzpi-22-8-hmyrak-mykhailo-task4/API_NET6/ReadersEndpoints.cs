...
{
    public static void MapReadersEndpoints(this IEndpointRouteBuilder endpoints, libraryContext db)
    {
        ...
        endpoints.MapGet("/readers/{id}", (int id) =>
        {
            var reader = db.Readers
                ...
                .Select(r => new
                {
                    ...
                    BorrowedBooks = r.Hists
                        .Where(h => h.StatusId == 2)
                        .Select(h => new
                        {
                            BorrowTime = h.Time,
                            ReturnTime = db.Hists
                                .Where(hr => hr.ItemId == h.ItemId && hr.StatusId == 3)
                                .Select(hr => (DateTime?)hr.Time)
                                .FirstOrDefault(),
                            h.ItemId,
                            BookTitle = h.Item.Book.Title
                        }).ToList()
                })
